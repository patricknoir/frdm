...
val settings = Seq(
  ...
  initialCommands in console :=
    """
      |import org.patricknoir.frdm._
      |
    """.stripMargin,
  ScalariformKeys.preferences := PreferencesImporterExporter.loadPreferences((file(".") / "formatter.preferences").getPath)
  //mainClass in (Compile, run) := Option("org.patricknoir.bet.Main")
)

val dockerSettings = Seq(
  defaultLinuxInstallLocation in Docker := "opt/frdm",
  dockerCommands := Seq(
    Cmd("FROM", "alpine:3.3"),
    Cmd("RUN apk upgrade --update && apk add --update openjdk-jre && rm -rf /var/cache/apk/*"),
    Cmd("ADD", "opt /opt"),
    ExecCmd("RUN", "mkdir", "-p", "/var/log/frdm"),
    ExecCmd("ENTRYPOINT", "/opt/frdm/bin/frdm")
  ),
